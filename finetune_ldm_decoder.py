...
def get_parser():
    ...
    aa("--train_dir", type=str, help="Path to the training data directory", default = "D:\python_projects\signature\stable_signature-main\coco\\train")
    aa("--val_dir", type=str, help="Path to the validation data directory", default = "D:\python_projects\signature\stable_signature-main\coco\\val")

    group = parser.add_argument_group('Model parameters')
    aa("--ldm_config", type=str, default="D:\python_projects\signature\stable_signature-main\\upload\\v1-inference.yaml", help="Path to the configuration file for the LDM model")
    aa("--ldm_ckpt", type=str, default="D:\python_projects\signature\stable_signature-main\\upload\sd-v1-4.ckpt", help="Path to the checkpoint file for the LDM model")
    aa("--msg_decoder_path", type=str, default= "D:/python_projects/signature/stable_signature-main/hidden/models/dec_48b_whit.torchscript.pt", help="Path to the hidden decoder for the watermarking model")
    ...
    aa("--lambda_i", type=float, default=0.7, help="Weight of the image loss in the total loss")
    aa("--lambda_w", type=float, default=0.3, help="Weight of the watermark loss in the total loss")
...
def main(params):
    ...
    for ii_key in range(params.num_keys):
        ...
        save_file(ldm_decoder.state_dict(),
                  os.path.join(params.output_dir, f"diffusion_pytorch_model.safetensors_{ii_key:03d}.pth"))
        print("模型已保存为 diffusion_pytorch_model.safetensors")
